...
def send_transfer_confirmation(sender_id, vk, peer_id):
    ...
    confirmation_msg = (f"Подтвердите перевод: вы ({sender_nickname}) хотите отправить {amount} Glitch игроку ({recipient_nickname}).")
    vk.messages.send(
        peer_id=peer_id,
...
def process_transfer_confirmation(event, player_data, vk, peer_id, sender_id, action):
    ...
    if action == "confirm":
        ...
        vk.messages.send(
            ...
            message=f"Вам переведен {amount} Glitch от {sender_nickname}.",
            random_id=random.randint(1, 1000)
        )
        del transfer_sessions[str(sender_id)]
        return